...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('Parcela ID: ' || v_ParcelaID);
    DBMS_OUTPUT.PUT_LINE('Parcela Name: ' || v_ParcelaName);

    FOR rec IN (
        SELECT TP.Designacao AS Tipo, COUNT(F.OperacaoID) AS numeroOperacoes
        FROM Operacao OP
        INNER JOIN Fertilizacao F ON OP.ID = F.OperacaoID
        INNER JOIN FatorDeProducao FP ON F.FatorDeProducaoID = FP.ID
        INNER JOIN TipoProduto TP ON FP.TipoProdutoID = TP.ID
        ...
        FROM Operacao OP
        INNER JOIN AplicacaoFitofarmaco AF ON OP.ID = AF.OperacaoID
        INNER JOIN FatorDeProducao FP ON AF.FatorDeProducaoID = FP.ID
        INNER JOIN TipoProduto TP ON FP.TipoProdutoID = TP.ID
        WHERE OP.ParcelaID = v_ParcelaID
        AND OP.DataOperacao BETWEEN p_StartDate AND p_EndDate
        GROUP BY TP.Designacao
